...
public class Kernel extends Thread {
  ...
  public static ArrayList<String> segmentos = new ArrayList<String>();
  ...
  public void init(String commands, String config) {
        ...
        if (line.startsWith("READ") || line.startsWith("WRITE")) {
          ...
          if (tmp.startsWith("random")) {
            ...
            try {
              ...
              long pageLow = Long.parseLong(newMemRange[0], numParser);
              long pageHigh = Long.parseLong(newMemRange[1], numParser);
              // System.out.println("First " + newMemRange[0]);
    ...
    for (i = 0; i < instructVector.size(); i++) {
      ...
      if (instruct.addr < 0 || instruct.addr > high) {
        System.out
            .println("MemoryManagement: Instruction (" + instruct.inst + " " + instruct.addr + ") out of bounds.");
        System.exit(-1);
      }
    }
  }
  ...
  public void step() {
    ...
    if (instruct.inst.startsWith("READ")) {
      System.out.println("Pag " + Virtual2Physical.pageNum(instruct.addr, virtPageNum, block));
     
      Page page = (Page) memVector.elementAt(Virtual2Physical.pageNum(instruct.addr, virtPageNum, block));
      try {

        segmentos = Virtual2Physical.segArray(Virtual2Physical.pageNum(instruct.addr, virtPageNum, block), block, instruct.low, instruct.high);
        page.segmentos = segmentos;
        controlPanel.segmentosValueLabel.setText( segmentos.toString() != null ? segmentos.toString() : "-1");
       } catch (Exception e) {
         System.err.println("Error en segArray  " + e);
       }
      ...
      if (page.physical == -1) {
        ...
        PageFault.replacePage(memVector, virtPageNum, Virtual2Physical.pageNum(instruct.addr, virtPageNum, block),
            controlPanel);
